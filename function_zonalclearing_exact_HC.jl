...
function market_clearing!(a::Model)
    ...
    vbar = a.ext[:variables][:vbar] = @variable(a, [g in G, t in T], lower_bound = 0, upper_bound = 1, base_name = "dispatch bar")
    ...
    F_FBMCbar = a.ext[:variables][:F_FBMCbar] = @variable(a, [l in L, t in T], base_name = "commercial flow bar")
    F_DC = a.ext[:variables][:F_DC] = @variable(a, [l_dc in L_DC, t in T], base_name = "commercial DC flow")
    F_DCbar = a.ext[:variables][:F_DCbar] = @variable(a, [l_dc in L_DC, t in T], base_name = "commercial DC flow bar")
    ...
    Fbar = a.ext[:expressions][:Fbar] = @expression(a, [l in L, t in T], sum(NPTDF[findfirst(N .== n),l]*(sum(GENCAP[g]*(vbar[g,t]) for g in g_in_n[n]) + (RES[t,findfirst(N .== n)]-curtbar[n,t])-DEM[t,findfirst(N .== n)] - sum(F_DCbar[l_dc,t]*inc_dc[l_dc,findfirst(N .== n)] for l_dc in L_DC)) for n in N ))
    ...
    a.ext[:constraints][:con2] = @constraint(a, con2[z in Z, t in T], sum(F[l,t]*i[l,z] for l in L) + sum(F_DC[l_dc,t]*i_DC[l_dc,z] for l_dc in L_DC) - p[z,t] == 0 )
    ...
    a.ext[:constraints][:con5] = @constraint(a, con5[z in Z, t in T], sum(Fbar[l,t]*i[l,z] for l in L) + sum(F_DCbar[l_dc,t]*i_DC[l_dc,z] for l_dc in L_DC) - p[z,t] == 0 )
    a.ext[:constraints][:con6] = @constraint(a, con6[l in L, t in T], -TC[l] <= Fbar[l,t] <= TC[l])
    a.ext[:constraints][:con7] = @constraint(a, con7[l_dc in L_DC, t in T], -TC_DC[l_dc] <= F_DCbar[l_dc,t] <= TC_DC[l_dc]) 
